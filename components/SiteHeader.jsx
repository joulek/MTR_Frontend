...
/* ✅ ADDED: slugify + makeCatHref (باش نخرّجو رابط صفحة الكاتغوري) */
function slugify(s = "") {
  return String(s)
...
export default function SiteHeader() {
  ...
  /* Locale depuis <html lang> / navigator */
  useEffect(() => {
    const pick = () => {
  ...
  /* Charger les catégories pour le menu Produits (comme sur la Home) */
  useEffect(() => {
    let alive = true;
    ...
    (async () => {
      try {
        ...
        // ⚠️ Ignorer TOUTES les annulations pour éviter l’overlay rouge
        const isAbort =
          err?.name === "AbortError" ||
        ...
        // Optionnel: log léger sans error overlay
        // console.warn("SiteHeader: chargement catégories échoué:", err);
      } finally {
          ...
          <div className="absolute left-0 top-full z-50 before:content-[''] before:absolute before:top-0 before:left-0 before:w-full before:h-2">
            ...
            <ul className="relative w-64 rounded-lg bg-white p-2 shadow-2xl ring-1 ring-slate-200 after:content-[''] after:absolute after:top-0 after:right-[-8px] after:w-2 after:h-full">
              {tops.map((parent) => {
                ...
                return (
                  <li key={id}>
                    {/* ✅ ADDED: redirection vers page produits de la catégorie */}
                    <a
            ...
            {hoveredParent && (childrenMap.get(hoveredParent) || []).length > 0 && (
              <ul className="absolute left-[100%] top-0 ml-2 w-64 rounded-lg bg-white p-2 shadow-2xl ring-1 ring-slate-200">
                {childrenMap.get(hoveredParent).map((child) => (
                  <li key={getId(child)}>
                    {/* ✅ ADDED: redirection pour sous-catégories */}
                    <a
      ...
      <div className="bg-[#0B2239] text-white">
        <div className="mx-auto flex h-9 max-w-7xl items-center justify-between px-4 text-xs sm:text-sm">
          <nav className="flex items-center gap-4">
            ...
            <span className="opacity-40">|</span>
            <a href="#^presentation" className="opacity-90 transition hover:text-[#F5B301]">Présentation</a>
          </nav>
        ...
        <div className="mx-auto max-w-7xl px-4">
          <div className="flex h-16 items-center justify-between">
            ...
            <nav className="hidden items-center gap-1 md:flex">
              <a href="/" className="px-3 py-2 text-sm font-semibold text-[#0B2239] hover:text-[#F5B301]">Accueil</a>
              <a href="#presentation" className="px-3 py-2 text-sm font-semibold text-[#0B2239] hover:text-[#F5B301]">L'entreprise</a>

              {!loadingCats && <ProductsMenu cats={categories} locale={locale} />}

              <a href="#contact" className="px-3 py-2 text-sm font-semibold text-[#0B2239] hover:text-[#F5B301]">Contact</a>
              <a href="#localisation" className="px-3 py-2 text-sm font-semibold text-[#0B2239] hover:text-[#F5B301]">Localisation</a>
            </nav>
        ...
        {open && (
          <div className="md:hidden border-t border-slate-200 bg-white">
            <div className="mx-auto max-w-7xl px-4 py-3 flex flex-col gap-1">
              <a href="/" className="rounded px-3 py-2 hover:bg-slate-50" onClick={() => setOpen(false)}>Accueil</a>
