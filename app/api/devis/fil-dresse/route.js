...
export async function POST(request) {
  try {
    ...
    console.log("→ Envoi au backend:", `${BACKEND}/api/devis/fil-dresse`);
    console.log("→ Token utilisé:", token.slice(0, 10) + "...");

    // proxy vers le backend (NE PAS définir Content-Type en multipart)
    const res = await fetch(`${BACKEND}/api/devis/fil-dresse`, {
      method: "POST",
      body: formData,
    ...
    console.error("proxy /api/devis/fil-dresse:", err);
    return NextResponse.json(
      { success: false, message: "Erreur serveur (proxy fil-dresse)" },
      { status: 500 }
    );
  }
}